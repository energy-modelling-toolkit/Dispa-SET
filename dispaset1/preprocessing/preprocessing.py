...
def build_simulation(config,plot_load=False):
    ...
    for zone in config['zones']:
        for fuel in fuels:
            try:
                ...
                FirstCell = tmp.columns[0]
            except:
                logging.warning("Couldn't load file of fuel prices for " + fuel)
            if os.path.isfile(config[fuel]) and isinstance(FirstCell, (int, long, float, complex)):
    ...
    fuel_series = list(fuels2*len(config['zones']))
    zone_series = []
    for zone in config['zones']:
        zone_series = zone_series+[zone]*len(fuels2)
    zonefuels_array = [zone_series, fuel_series]
    idx = pd.IndexSlice
    FirstCell = 0
    for zone in config['zones']:
        for fuel in fuels2:
            ...
            # FIXME: Currently the default values are assigned ignoring the loaded csv file
                ...
                FuelPrices.loc[idx[idx_utc_noloc], idx[zone, fuel]] = pd.Series(config['default'][fuel], index=idx_utc_noloc)
            # Special case for lignite and peat, for backward compatibility
            elif fuel == 'PriceOfLignite 2':
                logging.warn('No price data found for "' + fuel + '. Using the same value as for Black Coal')
                FuelPrices.loc[idx[idx_utc_noloc], idx[zone, fuel]] = FuelPrices[zone]['PriceOfBlackCoal 2']
            elif fuel == 'PriceOfPeat 2':
                logging.warn('No price data found for "' + fuel + '. Using the same value as for biomass')
                FuelPrices.loc[idx[idx_utc_noloc], idx[zone, fuel]] = FuelPrices[zone]['PriceOfBiomass 2']
            else:
                logging.warn('No data file or default value found for "' + fuel + '. Assuming zero marginal price!')
                FuelPrices.loc[idx[idx_utc_noloc], idx[zone, fuel]] = pd.Series(0, index=idx_utc_noloc)
    ####### Edits #######
    ...
    for unit in range(Nunits):
        ...
        for FuelEntry in FuelEntries2:
            if Plants_merged['Fuel'][unit] == FuelEntry:
                parameters['CostVariableB']['val'][unit, :] = FuelPrices[zone][FuelEntries2[FuelEntry]] / Plants_merged['Efficiency'][unit] + \
                                                             Plants_merged['EmissionRate'][unit] * FuelPrices[zone]['PriceOfCO2 2']
                found = True
        # Special case for biomass plants, which are not included in EU ETS:
        if Plants_merged['Fuel'][unit] == 'BIO':
