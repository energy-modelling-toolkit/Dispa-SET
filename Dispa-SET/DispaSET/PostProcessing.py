# -*- coding: utf-8 -*-
"""
Set of functions useful to analyse to Dispa-SET output data.

@author: Sylvain Quoilin, JRC
"""

from __future__ import division
from DispaSET_io_data import GdxToList,GdxToDataframe
from DispaTools import get_gams_path,shrink_to_64
import pandas as pd
import numpy as np
import pickle
import sys
...
def colormap(f):
    ''' 
    Function that associates a color to each dispa-SET fuel for plotting
    ...
    '''
    colors = {'NUC':'orange','LIG':'brown','HRD':'grey','BIO':'darkgreen','GAS':'lightcoral','OIL':'chocolate','WST':'dodgerblue','SUN':'yellow','WIN':'red','FlowIn':'green','WAT':'blue','Storage':'blue','FlowOut':'green'}
    return colors[f]

def UnitLocation(Inputs,shrink=True):
    ''' 
...
def UnitFuel(Inputs,shrink=True):
    ''' 
...
def AggregateByFuel(PowerOutput,Inputs,SpecifyFuels=None):
    ...
    uFuel = UnitFuel(Inputs)
    
    for u in PowerOutput:
        if uFuel[u] in fuels:
            PowerByFuel[uFuel[u]] = PowerByFuel[uFuel[u]] + PowerOutput[u]
        else:
            print 'Fuel not found for unit ' + u + ' with fuel ' + uFuel[u]
            
    return PowerByFuel
    
def FilterByCountry(PowerOutput,inputs,c):
    '''
    ...
    '''     
    loc = UnitLocation(inputs)
    Power = PowerOutput.loc[:,[u for u in PowerOutput.columns if loc[u]==c]]
    return Power
    
def GetPlotData(inputs,results,c):
    '''
    Function that reads the results dataframe of a Dispa-SET simulation and extract the dispatch data spedific to one country
    ...
    '''
    tmp = FilterByCountry(results['OutputPower'],inputs,c)
    plotdata = AggregateByFuel(tmp,inputs)
    
    if 'OutputStorageInput' in results:
        tmp = FilterByCountry(results['OutputStorageInput'],inputs,c)
...
#            plotdata[col+'_pump'] = -tmp[col]
    ...
    # re-ordering columns:
    plotdata = plotdata[[ u'Storage', u'FlowOut', u'NUC', u'LIG', u'HRD', u'BIO', u'GAS', u'OIL', u'WST', u'SUN', u'WIN', u'FlowIn', u'WAT']]

    return plotdata    

def dispatch_plot(demand,plotdata,level=None,rng=[]):
    '''
    ...
    import itertools
    # loop through colors. 
    colors = itertools.cycle([colormap(x) for x in plotdata.columns])    
...
def plot_rug(df_series, on_off=False, cmap='Greys'):
    ...
    """
    from DispaTools import clean_strings
...
def EnergyBarPlot(datain,results,PPindicators):
    '''
    ...
    for c in countries:
        ...
        NetImports = imports(results['OutputFlow'],c)
        if NetImports > 0:
            GenPerCountry.loc[c,'FlowIn'] = NetImports   
    
    GenPerCountry = GenPerCountry[['NUC','LIG','HRD','BIO','GAS','OIL','WST','SUN','WIN','FlowIn','WAT']]/1E6
    colors = [colormap(tech) for tech in GenPerCountry.columns]
    ax = GenPerCountry.plot(kind="bar", figsize=(12,8),stacked=True, color=colors, alpha=0.8,legend='reverse',title='Generation per country (the horizontal lines indicate the demand)')
    ax.set_ylabel('Generation [TWh]')
    demand = datain['Demand']['DA'].sum()/1E6
    ax.barh(demand,left=ax.get_xticks()-0.4, width=[0.8]*len(demand),height=demand.values*0,linewidth=2,color='k')
    return ax

def GetResults(path='.',cache = False, TempPath='.pickle'):
    '''
    ...
    :param TempPath:            Temporary path to store the cache file
    ...
    if cache:
        ...
        filepath_pickle = str(TempPath + os.path.sep + resultfile_hash + '.p')
        ...
        if os.path.getmtime(resultfile) > time_pd:
            results = GdxToDataframe(GdxToList(gams_dir,resultfile,varname='all',verbose=True),fixindex=True,verbose=True)
    ...
    for key in ['OutputPower','OutputSystemCost','OutputCommitted','OutputCurtailedPower','OutputFlow', 'OutputShedLoad','OutputSpillage','OutputStorageLevel','OutputStorageInput','LostLoad_Reserve2U', 'LostLoad_MaxPower','LostLoad_MinPower','LostLoad_RampUp','LostLoad_RampDown','LostLoad_Reserve2D','ShadowPrice']:
        if key in results:
            if len(results[key]) == len(index_long):                # Case of variables for which the look-ahead period recorded (e.g. the lost loads)
                ...
                if key in ['OutputPower','OutputSystemCost','OutputCommitted','OutputCurtailedPower','OutputFlow', 'OutputShedLoad','OutputSpillage','OutputStorageLevel','OutputStorageInput']:
    ...
    for key in results['OutputPower']:
        if key not in inputs['units'].index:
            warn("Unit '" + key + "' present in the results cannot be found in the input 'units' dataframe")
...
def CountryPlots(inputs,results,c,rng=[]):
    '''
    ...
    plotdata = GetPlotData(inputs,results,c)
    
    if 'OutputStorageLevel' in results:
        level = FilterByCountry(results['OutputStorageLevel'],inputs,c)
        level = level.sum(axis=1)
    else:
        level = pd.Series(0,index=results['OutputPower'].index)
    
    demand = GetDemand(inputs,c)
    sum_generation = plotdata.sum(axis=1)
    
    dispatch_plot(demand,plotdata,level,rng=rng)
    
    # Generation plot: 
    CountryGeneration = FilterByCountry(results['OutputPower'],inputs,c)
...
#%%
def ResultAnalysis(inputs,results):
    '''
    Reads the Dispa-SET results and provides useful general information to stdout

    :param inputs:      Dispa-SET inputs
    :param results:     Dispa-SET results
    '''

    from DispaTools import ds_to_df
    ...
    NetImports = -imports(results['OutputFlow'],'RoW')

    Cost_kwh =results['OutputSystemCost'].sum()/ (TotalLoad - NetImports)
    
    print '\nAverage electricity cost : ' + str(Cost_kwh) + ' EUR/MWh'
    
    for key in ['LostLoad_RampUp', 'LostLoad_Reserve2D','LostLoad_MinPower','LostLoad_RampDown','LostLoad_Reserve2U', 'LostLoad_MaxPower']:
    ...
    for c in CountryData.index:
        CountryData.loc[c,'NetImports'] = imports(results['OutputFlow'],str(c))/1E6
    ...
    if 'OutputFlow' in results:
        for l in results['OutputFlow']:
            if l[:3] != 'RoW' and l[-3:] != 'RoW':
                Congestion[l] = np.sum((results['OutputFlow'][l]==dfin['FlowMaximum'].loc[results['OutputFlow'].index,l]) & (dfin['FlowMaximum'].loc[results['OutputFlow'].index,l]>0))
    print "\nNumber of hours of congestion on each line: "
    import pprint
    pprint.pprint(Congestion)
    return {'Cost_kwh':Cost_kwh,'TotalLoad':TotalLoad,'PeakLoad':PeakLoad,'NetImports':NetImports,'CountryData':CountryData,'Congestion':Congestion}
#%%

def PerPowerPlantIndicators(inputs,results):
    '''
