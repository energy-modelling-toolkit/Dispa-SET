...
*Get the numeric value of the h set (ord function should not be used because h can be wrongly ordered)
parameter idx(h),pos(h);
idx(h) = h.val;
...
EQ_CostStartUp(u,i)$(CostStartUp(u) <> 0)..
         CostStartUpH(u,i)
         =g=
         CostStartUp(u)*(Committed(u,i)-CommittedInitial(u)$(pos(i) = 1)-Committed(u,i-1)$(pos(i) > 1))
;

EQ_CostShutDown(u,i)$(CostShutDown(u) <> 0)..
         CostShutDownH(u,i)
         =g=
         CostShutDown(u)*(CommittedInitial(u)$(pos(i) = 1)+Committed(u,i-1)$(pos(i) > 1)-Committed(u,i))
;

EQ_CostRampUp(u,i)$(CostRampUp(u) <> 0)..
         CostRampUpH(u,i)
         =G=
         CostRampUp(u)*(Power(u,i)-PowerInitial(u)$(pos(i) = 1)-Power(u,i-1)$(pos(i) > 1))
;

EQ_CostRampDown(u,i)$(CostRampDown(u) <> 0)..
         CostRampDownH(u,i)
         =g=
         CostRampDown(u)*(PowerInitial(u)$(pos(i) = 1)+Power(u,i-1)$(pos(i) > 1)-Power(u,i))
;
...
EQ_Ramp_up(u,i)$(sum(tr,Technology(u,tr))=0)..
         ...
         +RampStartUpMaximum(u)
                 *(1-CommittedInitial(u)))$(pos(i) = 1)
         +(Power(u,i-1)
         +RampUpMaximum(u)
                 *Committed(u,i-1)
         +RampStartUpMaximum(u)
                 *(1-Committed(u,i-1)))$(pos(i) > 1)
         +LostLoad_RampUp(u,i)
;
...
EQ_Ramp_down(u,i)$(sum(tr,Technology(u,tr))=0)..
         (PowerInitial(u)-Power(u,i))$(pos(i) = 1)
         +(Power(u,i-1)-Power(u,i))$(pos(i) > 1)
         =L=
...
*         Committed(u,i)
*         =G=
*         sum(ii$((pos(ii) >= pos(i)+1-TimeUpMinimum(u)) and (pos(ii))<=pos(i))),Committed(u,i)-CommittedH(h-1,u))
*;

EQ_Minimum_time_up_A(u)..
         sum(i$(pos(i) <= TimeUpLeft_initial(u)),1-Committed(u,i))
         =E=
         0
;

EQ_Minimum_time_up_B(u,i)$((TimeUpLeft_initial(u)+1 <= pos(i)) and (pos(i) <= card(i)-TimeUpMinimum(u)+1))..
         sum(ii$((pos(i) <= pos(ii)) and (pos(ii) <= (pos(i)+TimeUpMinimum(u)-1))),Committed(u,ii))
         =G=
         TimeUpMinimum(u)
                 *(Committed(u,i)-CommittedInitial(u)$(pos(i) = 1)-Committed(u,i-1)$(pos(i) > 1))
;

EQ_Minimum_time_up_C(u,i)$((card(i)-TimeUpMinimum(u)+2 <= pos(i)) and (pos(i)<=card(i)))..
         sum(ii$((pos(i) <= pos(ii)) and (pos(ii) <= card(i))),Committed(u,ii)-(Committed(u,i)-CommittedInitial(u)$(pos(i) = 1)-Committed(u,i-1)$(pos(i) > 1)))
         =G=
         0
;

EQ_Minimum_time_up_JustStarted(u,i)$(pos(i) > 1)..
         sum(ii$((pos(i) <= pos(ii)) and (pos(ii) <= (pos(i)+TimeUpLeft_JustStarted(u,i)-1))),Committed(u,ii))
         =G=
...
*         1-Committed(u,i)
*         =G=
*         sum(ii$((pos(ii) >= idx(h)+1-TimeDownMinimum(u)) and (pos(ii))<=idx(h))),CommittedH(h-1,u)-Committed(u,i))
*;

EQ_Minimum_time_down_A(u)..
         sum(i$(pos(i) <= TimeDownLeft_initial(u)),Committed(u,i))
         =E=
         0
;

EQ_Minimum_time_down_B(u,i)$((TimeDownLeft_initial(u)+1 <= pos(i)) and (pos(i) <= card(i)-TimeDownMinimum(u)+1))..
         sum(ii$((pos(i) <= pos(ii)) and (pos(ii) <= (pos(i)+TimeDownMinimum(u)-1))),1-Committed(u,ii))
         =G=
         TimeDownMinimum(u)
                 *(CommittedInitial(u)$(pos(i) = 1)+Committed(u,i-1)$(pos(i) > 1)-Committed(u,i))
;

EQ_Minimum_time_down_C(u,i)$((card(i)-TimeDownMinimum(u)+2 <= pos(i)) and (pos(i)<=card(i)))..
         sum(ii$((pos(i) <= pos(ii)) and (pos(ii) <= card(i))),1-Committed(u,ii)-(CommittedInitial(u)$(pos(i) = 1)+Committed(u,i-1)$(pos(i) > 1)-Committed(u,i)))
         =G=
         0
;

*IH: why do we need this equation?, to replace the two previous?
EQ_Minimum_time_down_JustStopped(u,i)$(TimeDownLeft_initial(u)+1 <= pos(i))..
         sum(ii$((pos(i) <= pos(ii)) and (pos(ii) <= (pos(i)+TimeDownLeft_JustStopped(u,i)-1))),1-Committed(u,ii))
         =G=
...
EQ_Storage_balance(s,i)..
         StorageInitial(s)$(pos(i) = 1)
         +StorageLevel(s,i-1)$(pos(i) > 1)
*         +StorageLevelH(h--1,s)
         +StorageInflow(s,i)
         +StorageInput(s,i)*StorageChargingEfficiency(s)
         =E=
...
EQ_Storage_boundaries(s,i)$(pos(i) = card(i))..
         StorageFinalMin(s)
         =L=
         StorageLevel(s,i)
;
...
EQ_Force_Commitment(u,i)$((sum(tr,Technology(u,tr))>=1 and LoadMaximum(u,i)>0) or (idx(i)=4 and ord(u)=129))..
         Committed(u,i)
         =E=
         1;
...
FOR(day = 1 TO ndays-Config("RollingHorizon LookAhead","day") by Config("RollingHorizon Length","day"),
         ...
         i(h) = no;
         i(h)$(idx(h)>=firsthour and idx(h)<=lasthour)=yes;
         pos(i) = idx(i) - FirstHour + 1  ;

         TimeUpLeft_initial(u)=max(0,min(card(i),(TimeUpMinimum(u)-TimeUpInitial(u))*CommittedInitial(u)));
         TimeUpLeft_JustStarted(u,i) = max(0,min(card(i)-pos(i)+1,TimeUpMinimum(u)));
         TimeDownLeft_initial(u)=max(0,min(card(i),(TimeDownMinimum(u)-TimeDownInitial(u))*(1-CommittedInitial(u))));
         TimeDownLeft_JustStopped(u,i) = max(0,min(card(i)-pos(i)+1,TimeDownMinimum(u)));

         StorageFinalMin(s) =  min(StorageInitial(s) + sum(i,StorageInflow(s,i)) - sum(i,StorageOutflow(s,i)) , sum(i$(pos(i)=card(i)),StorageProfile(s,i)*StorageCapacity(s)*AvailabilityFactor(s,i)));

         display day,FirstHour,LastHour,LastKeptHour,TimeDownInitial,TimeUpLeft_JustStarted,TimeDownLeft_JustStopped,i,pos;
...
*Time counters
         Loop(i,
              TimeUp(u,i)$(idx(i) = 1 and Committed.L(u,i) = 1)=TimeUpInitial(u)+1;
              TimeUp(u,i)$(idx(i) = 1 and Committed.L(u,i) = 0)=0;
              TimeUp(u,i)$(idx(i) > 1 and Committed.L(u,i) = 1) = TimeUp(u,i-1)+1;
              TimeUp(u,i)$(idx(i) > 1 and Committed.L(u,i) = 0) = 0;

              TimeDown(u,i)$(idx(i) = 1 and Committed.L(u,i) = 0) = TimeDownInitial(u)+1;
              TimeDown(u,i)$(idx(i) = 1 and Committed.L(u,i) = 1) = 0;
              TimeDown(u,i)$(idx(i) > 1 and Committed.L(u,i) = 0) = TimeDown(u,i-1)+1;
              TimeDown(u,i)$(idx(i) > 1 and Committed.L(u,i) = 1) = 0;
              );

         TimeUpInitial(u)=sum(i$(idx(i)=LastKeptHour-FirstHour+1),TimeUp(u,i));
         TimeDownInitial(u)=sum(i$(idx(i)=LastKeptHour-FirstHour+1),TimeDown(u,i));
         CommittedInitial(u)=sum(i$(idx(i)=LastKeptHour-FirstHour+1),Committed.L(u,i));
         PowerInitial(u) = sum(i$(idx(i)=LastKeptHour-FirstHour+1),Power.L(u,i));

         StorageInitial(s) =   sum(i$(idx(i)=LastKeptHour-FirstHour+1),StorageLevel.L(s,i));
         Display TimeDown,Committed.L,Power.L;
);
