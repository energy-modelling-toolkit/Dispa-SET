...
def get_result_analysis(inputs, results):
    ...
    try:
        ...
        for c in StorageData.index:
            ...
            StorageData.loc[c, 'Peak load shifting [hours]'] = StorageData.loc[c, 'Storage Capacity [MWh]'] / \
                                                               CountryData.loc[c, 'PeakLoad']
            ...
            StorageData.loc[c, 'Average daily cycle depth [%]'] = AverageStorageOutput * 24 / StorageData.loc[
                c, 'Storage Capacity [MWh]']
        print('\nCountry-Specific storage data')
        print(StorageData)
    except:
...
def CostExPost(inputs, results):
    ...
    # %% Lost loads:
    costs['LostLoad'] = 80e3 * (
    results['LostLoad_2D'].reindex(timeindex).sum(axis=1).fillna(0) + results['LostLoad_2U'].reindex(timeindex).sum(
        axis=1).fillna(0) + results['LostLoad_3U'].reindex(timeindex).sum(axis=1).fillna(0)) \
                        + 100e3 * (
    results['LostLoad_MaxPower'].reindex(timeindex).sum(axis=1).fillna(0) + results['LostLoad_MinPower'].reindex(
        timeindex).sum(axis=1).fillna(0)) \
                        + 70e3 * (
    results['LostLoad_RampDown'].reindex(timeindex).sum(axis=1).fillna(0) + results['LostLoad_RampUp'].reindex(
        timeindex).sum(axis=1).fillna(0))
