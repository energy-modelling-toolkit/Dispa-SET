...
from .data_handler import NodeBasedTable, load_time_series, UnitBasedTable, merge_series, define_parameter, \
    load_geo_data, GenericTable, load_config, normalize_reserve_config
...
def build_single_run(config, profiles=None, PtLDemand=None, SectorXFlexDemand=None, SectorXFlexSupply=None, MTS=0,
    ...
    if profilesSectorX is not None and not profilesSectorX.empty:
        # Keeping consistency in profiles columns names
        profilesSectorX.columns = profilesSectorX.columns.astype(str).str.split(" - ").str[-1].str.strip()
