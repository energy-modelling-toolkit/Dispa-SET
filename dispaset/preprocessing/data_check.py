    ...
    for z in Load.columns:
        if z in aFRRUDemand:
            ...
            if (Load[z] - aFRRUDemand[z] < 0).any():
                msg = 'The reserve aFRRU table contains negative values higher than demand for zone ' + z
                logging.critical(msg)
                raise DispaSETValidationError(msg)
        else:
...
def check_VIRDemand(VIRDemand, Load):
    ...
    if (VIRDemand.sum(axis=1) < 0).any():
        msg = 'The VIR Demand table contains negative values'
        logging.critical(msg)
        raise DispaSETValidationError(msg)
    if (Load.sum(axis=1) - VIRDemand.sum(axis=1) < 0).any():
        msg = 'The VIR Demand table contains values higher than demand'
        logging.critical(msg)
        raise DispaSETValidationError(msg)
    else:
